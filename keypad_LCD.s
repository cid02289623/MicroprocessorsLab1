...
; NOTE: assumes you manage cursor state yourself if using this
; ------------------------------------------------------------
LCD_Write_Message:
    movwf   LCD_counter, A
LCD_Loop_message:
...
; ------------------------------------------------------------
; Write a single ASCII char, but DO NOT go past col 15
; W = ASCII character to write
; ------------------------------------------------------------
; ------------------------------------------------------------
; Write a single ASCII char with auto-wrap:
; - Writes at current cursor position
; - Advances cursor
; - If at end of line 1 (col 15, row 0): jump to start of line 2
; - If at end of line 2 (col 15, row 1): stay at col 15 (no wrap)
; Inputs: W = ASCII char
; Uses: LCD_row (0/1), LCD_col (0..15)
; ------------------------------------------------------------
; ------------------------------------------------------------
; LCD_Write_Char with full 2-line wrap
; - Writes char
; - Advances cursor
...
LCD_Write_Char:
    ...
    ; --- col == 15 (end of line) ---
    movf    LCD_row, F, A
    bz      _wrap_to_line2      ; if row 0

    ; else row 1 → wrap to line 1
_wrap_to_line1:
    call    LCD_first_line      ; resets row=0, col=0
    return

_wrap_to_line2:
    call    LCD_second_line     ; sets row=1, col=0
    return
...
_write_and_inc:
    ; if LCD_col > 15 (shouldn't), refuse
    movlw   15
    cpfsgt  LCD_col, A          ; skip next if LCD_col > 15
    bra     _ok_write
    return
...
;; ------------------------------------------------------------
; Move cursor LEFT by 1 with wrap:
;   line1 col0  -> line2 col15
;   line2 col0  -> line1 col15
; ------------------------------------------------------------
LCD_shift_left_crsr:
    ; at col 0?
    movf    LCD_col, F, A
    ...
    ; col == 0, so wrap to other line col15
    movlw   15
    movwf   LCD_col, A

    movf    LCD_row, F, A
    bz      _l_from_line1       ; if row==0, wrap to line2
    ; else row==1, wrap to line1
_l_from_line2:
    clrf    LCD_row, A
    movlw   0x8F                ; line1 col15
    call    LCD_Send_Byte_I
    movlw   10
    call    LCD_delay_x4us
    return
...
_l_normal:
    ; normal move left within line
    movlw   0x10                ; cursor move left
    call    LCD_Send_Byte_I
    movlw   10
    call    LCD_delay_x4us
    decf    LCD_col, F, A
...
; ------------------------------------------------------------
; Move cursor RIGHT by 1 with wrap:
;   line1 col15 -> line2 col0
;   line2 col15 -> line1 col0
; ------------------------------------------------------------
LCD_shift_right_crsr:
    ...
    ; col == 15, wrap to other line col0
    clrf    LCD_col, A

    movf    LCD_row, F, A
    bz      _r_from_line1       ; if row==0, wrap to line2
    ; else row==1, wrap to line1
_r_from_line2:
    clrf    LCD_row, A
    movlw   0x80                ; line1 col0
    call    LCD_Send_Byte_I
    movlw   10
    call    LCD_delay_x4us
    return
...
_r_normal:
    ; normal move right within line
    movlw   0x14                ; cursor move right
    call    LCD_Send_Byte_I
    movlw   10
    call    LCD_delay_x4us
    incf    LCD_col, F, A
